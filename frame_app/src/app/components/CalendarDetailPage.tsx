...
function mapWebEvent(ev: WebCalendarEvent) {
  ...
  return {
    ...
    color: '#3b82f6',
    assignee: getUserLabel(ev.source_email),
...
export function CalendarDetailPage({ onClose }: CalendarDetailPageProps) {
  ...
  const [events, setEvents] = useState<ReturnType<typeof mapWebEvent>[]>([]);
  ...
  const fetchMonthEvents = useCallback(async () => {
    ...
    try {
      ...
      setEvents(items.map(mapWebEvent));
    } catch {
      setEvents([]);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('确定要删除该日程吗？删除后不可恢复。')) return;
    try {
