...
app.get('/api/web/events', async (req, res) => {
  try {
    ...
    // 查询 web_calendar_events
    let webQuery = 'SELECT * FROM web_calendar_events WHERE status = $1';
    const webParams = ['active'];
    let idx = 2;

    if (startAt) {
      webQuery += ` AND start_at >= $${idx++}`;
      webParams.push(startAt);
    }
    if (endAt) {
      webQuery += ` AND end_at <= $${idx++}`;
      webParams.push(endAt);
    }
    if (sourceEmail) {
      webQuery += ` AND source_email = $${idx++}`;
      webParams.push(sourceEmail);
    }
    webQuery += ' ORDER BY start_at ASC';

    const webResult = await pool.query(webQuery, webParams);

    // 同时查询 calendar_events（Flutter App 数据），映射为统一格式
    let calQuery = `SELECT ce.id, ce.title, ce.description, ce.start_at, ce.end_at, ce.location, 'manual' as provider, 
                    ce.user_id::text as source_app_user_id, COALESCE(u.email, '') as source_email, 'active' as status,
                    ce.created_at as synced_at, ce.created_at, ce.updated_at
                    FROM calendar_events ce
                    LEFT JOIN users u ON ce.user_id = u.id WHERE 1=1`;
    const calParams = [];
    idx = 1;

    if (startAt) {
      calQuery += ` AND start_at >= $${idx++}`;
      calParams.push(startAt);
    }
    if (endAt) {
      calQuery += ` AND end_at <= $${idx++}`;
      calParams.push(endAt);
    }
    calQuery += ' ORDER BY start_at ASC';

    const calResult = await pool.query(calQuery, calParams);

    // 合并去重（按 id 前缀区分）
    const seen = new Set();
    const merged = [];

    for (const r of webResult.rows) {
      const key = `web-${r.id}`;
      seen.add(key);
      merged.push({ ...r, id: `web-${r.id}`, _source: 'web' });
    }
    for (const r of calResult.rows) {
      const key = `cal-${r.id}`;
      if (!seen.has(key)) {
        seen.add(key);
        merged.push({ ...r, id: `cal-${r.id}`, _source: 'app' });
      }
    }
...
app.put('/api/web/events/:id/complete', async (req, res) => {
  try {
    ...
    // 解析前缀和真实 ID
    let realId, sourceTable;
    if (rawId.startsWith('web-')) {
      realId = parseInt(rawId.slice(4));
      sourceTable = 'web_calendar_events';
    } else if (rawId.startsWith('cal-')) {
      realId = parseInt(rawId.slice(4));
      sourceTable = 'calendar_events';
    } else {
      realId = parseInt(rawId);
      sourceTable = 'web_calendar_events';
    }
    if (isNaN(realId)) {
    ...
    if (sourceTable === 'web_calendar_events') {
      // 获取当前事件
      const currentResult = await pool.query('SELECT * FROM web_calendar_events WHERE id = $1', [realId]);
      ...
      if (newStatus === 'completed') {
        await pool.query(
          `INSERT INTO user_completed_events (event_id, source_app_user_id, source_email, title, completed_at)
           VALUES ($1, $2, $3, $4, NOW())`,
          [realId, event.source_app_user_id, event.source_email, event.title]
        );
      }

      res.json({ success: true, status: newStatus });
    } else {
      // calendar_events 表暂不支持完成状态切换
      res.json({ success: false, status: 'active', message: 'App 端日程暂不支持完成标记' });
    }
  } catch (error) {
...
app.delete('/api/web/events/:id', async (req, res) => {
  try {
    ...
    if (rawId.startsWith('web-')) {
      ...
      return res.status(400).json({ error: 'App 端日程暂不支持从 Web 删除' });
    } else {
      realId = parseInt(rawId);
    }
    if (isNaN(realId)) {
