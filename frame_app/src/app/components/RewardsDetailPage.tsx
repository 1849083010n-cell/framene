...
export function RewardsDetailPage({ onClose }: RewardsDetailPageProps) {
  ...
  const handleDeleteRecord = async (id: string, title: string) => {
    if (!window.confirm(`确定要删除「${title}」的完成记录吗？`)) return;
    try {
      await webEventsApi.removeCompletedRecord(id);
      if (viewingUser) {
        const res = await webEventsApi.getCompletedRecords(viewingUser);
        setSelectedUserRecords(res.items);
      }
      loadData();
    } catch (err) {
      console.error('删除完成记录失败', err);
    }
  };
